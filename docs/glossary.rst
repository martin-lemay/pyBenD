...
- Centerline: The centerline of a channel is the curve that runs along the middle of the
...
- Meander Bend (or half-meander): A meander bend is a curved section of a river or stream
...
- Bend Roundess: The roundness is the ratio of maximum to mean curvature along the bend
...
migration rate over channel aggradation rate (see ).
...
- Centerline collection: A centerline collection is a set of channel centerlines that
represent the same channel at different time steps. In pyBenD, centerline collections are
...
- Bend evolution: A bend evolution if a set of meander bends that represent the same bend
at different time steps. In pyBenD, bend evolutions are used to analyze the morphological
and kinematic changes of meander bends over time.
:class:`~pybend.model.BendEvolution.BendEvolution`

.. _glossary-channel-point:

- Channel point: A channel point is a point along the discretized channel centerline.
...
... and D’Alpaos, A. (2024b). Vegetation enhances curvature-driven dynamics in meandering
