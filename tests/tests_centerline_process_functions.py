...
from pybend.algorithms.synthetic_bends import circular_bend, mirror
...
lx2: npt.NDArray[np.float64] = np.array([-0.9906, -1.5219, -1.8323, -1.617, -0.556, 1.4964, 4.2984, 7.2383, 9.677, 11.2984, 12.1511, 12.4633, 12.4619, 12.2894, 11.9982, 11.5892, 11.0714])
ly2: npt.NDArray[np.float64] = np.array([5.9167, 8.8691, 11.8514, 14.8376, 17.6293, 19.7877, 20.782, 20.3342, 18.6225, 16.1134, 13.2434, 10.2619, 7.2625, 4.2677, 1.282, -1.6898, -4.6447])
...
lx_out: npt.NDArray[np.float64] = np.array(
    (0.0, 0.12, 0.278, 0.467, 0.682, 0.916, 1.163, 1.416, 1.669, 1.916, 2.151, 2.367, 2.565, 2.75,
     2.926, 3.099, 3.272, 3.45, 3.635, 3.83, 4.035, 4.251, 4.481, 4.722, 4.968, 5.211, 5.443, 5.656,
     5.848, 6.027, 6.198, 6.367, 6.541, 6.726, 6.924, 7.135, 7.355, 7.581, 7.81, 8.04, 8.27, 8.497,
     8.719, 8.935, 9.143, 9.341, 9.528, 9.701, 9.859, 10.0,)
)
ly_out: npt.NDArray[np.float64] = np.array(
    (0.0, 0.239, 0.442, 0.61, 0.744, 0.842, 0.907, 0.937, 0.933, 0.896, 0.825, 0.721, 0.588,
     0.432, 0.257, 0.071, -0.122, -0.316, -0.502, -0.67, -0.811, -0.914, -0.969, -0.972, -0.928, -0.845,
     -0.732, -0.597, -0.446, -0.282, -0.109, 0.069, 0.25, 0.429, 0.601, 0.755, 0.88, 0.964,
     0.998, 0.976, 0.903, 0.789, 0.642, 0.472, 0.288, 0.097, -0.09, -0.265, -0.419, -0.544,)
)

lx2_out: npt.NDArray[np.float64] = np.array(
    [-0.9906, -1.1698, -1.3466, -1.5109, -1.6526, -1.7613, -1.8271, -1.8394, -1.7862, -1.6549, 
     -1.4334, -1.113, -0.6868, -0.149, 0.4972, 1.2436, 2.081, 2.991, 3.9494, 4.9317, 5.913, 
     6.868, 7.7721, 8.6079, 9.3643, 10.0306, 10.6035, 11.086, 11.4816, 11.7966, 12.0399, 
     12.2209, 12.3483, 12.4304, 12.4757, 12.4909, 12.4813, 12.4517, 12.4059, 12.3458, 12.2728, 
     12.1881, 12.0912, 11.9818, 11.8596, 11.725, 11.5784, 11.4203, 11.2511, 11.0714]
)
ly2_out: npt.NDArray[np.float64] = np.array(
    [5.9167, 6.8751, 7.8362, 8.8004, 9.7678, 10.7388, 11.7137, 12.6921, 13.6693, 14.6392, 15.5946, 
     16.5222, 17.4057, 18.2286, 18.9711, 19.6116, 20.1292, 20.5087, 20.7385, 20.8077, 20.7167, 
     20.4731, 20.0852, 19.5672, 18.938, 18.2167, 17.4207, 16.5664, 15.6699, 14.7434, 13.7954, 
     12.834, 11.8649, 10.891, 9.9147, 8.9378, 7.961, 6.9845, 6.0087, 5.0337, 4.0596, 3.0865, 
     2.1145, 1.1438, 0.1747, -0.7928, -1.7586, -2.7226, -3.6846, -4.6447]
)
...
class TestsProcessFunctions(unittest.TestCase):
    ...
    def test_find_inflection_points(self: Self) -> None:
        ...
        curv: npt.NDArray[np.float64] = np.sin(np.linspace(1, 100., 100) / 10.)
    ...
    def test_find_inflection_points_from_peaks(self: Self) -> None:
        ...
        curv = np.sin(np.linspace(1, 100, 100) / 10.)
